#!/usr/bin/env python3
"""Cross-platform installation and video generation for the BuildshortVideos Skill."""
...
PROJECT_ARCHIVE_URL = (
    "https://github.com/ikppramesh/BuildshortVideos/archive/refs/heads/main.zip"
)
DEFAULT_ROOT = Path.home() / "BuildshortVideos"
...
RECOMMENDED_LLM_PROVIDERS = {
    "moonshot": (
        "Kimi / Moonshot AI",
        "https://platform.kimi.com/console/api-keys?aff=BuildshortVideos",
    ),
    ...
    "volcengine": (
        ...
        "https://www.volcengine.com/activity/ai618?utm_source=BuildshortVideos",
    ),
...
def log(message: str) -> None:
    ...
    print(f"[BuildshortVideos] {message}", flush=True)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install BuildshortVideos and generate a final video from a topic."
    )
    ...
    parser.add_argument(
        ...
        help=f"BuildshortVideos installation directory (default: {DEFAULT_ROOT})",
    )
    parser.add_argument(
        "cli_args",
        nargs=argparse.REMAINDER,
        help="additional BuildshortVideos CLI arguments placed after --",
    )
...
def ensure_project(root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mpt-install-") as temp_dir_value:
        ...
        archive_path = temp_dir / "BuildshortVideos.zip"
        request = urllib.request.Request(
            PROJECT_ARCHIVE_URL,
            headers={"User-Agent": "BuildshortVideos-Agent-Skill"},
        )
        ...
        if len(candidates) != 1:
            raise SkillError("download completed but no valid BuildshortVideos project was found")
        if root.exists():
            root.rmdir()
...
def _validate_pexels_key(api_key: str) -> str:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "BuildshortVideos-Agent-Skill",
        },
    )
    try:
...
def result_manifest_path(root: Path) -> Path:
    return root / ".agent-logs" / "buildshortvideos-video" / "latest-result.json"
...
def generate_video(
    ...
    log_dir = root / ".agent-logs" / "buildshortvideos-video"
