...
def _inject_env_into_config(cfg: dict) -> None:
    """Overlay .env API keys on top of config.toml values."""
    app_cfg = cfg.setdefault("app", {})

    env_map = {
        "PEXELS_API_KEY": ("pexels_api_keys", "list"),
        "PIXABAY_API_KEY": ("pixabay_api_keys", "list"),
        "COVERR_API_KEY": ("coverr_api_keys", "list"),
        "GEMINI_API_KEY": ("gemini_api_key", "str"),
        "OPENAI_API_KEY": ("openai_api_key", "str"),
        "DEEPSEEK_API_KEY": ("deepseek_api_key", "str"),
        "QWEN_API_KEY": ("qwen_api_key", "str"),
        "MOONSHOT_API_KEY": ("moonshot_api_key", "str"),
        "AZURE_OPENAI_API_KEY": ("azure_api_key", "str"),
    ...
    for env_key, (cfg_key, kind) in env_map.items():
        ...
        if not val:
            continue
        if kind == "list":
            existing = app_cfg.get(cfg_key) or []
            if val not in existing:
                app_cfg[cfg_key] = existing + [val]
        else:
            app_cfg[cfg_key] = val
...
def load_config():
    ...
    # fix: IsADirectoryError: [Errno 21] Is a directory: '/BuildshortVideos/config.toml'
    ...
    _inject_env_into_config(_config_)
    return _config_
...
project_name = _cfg.get("project_name", "BuildshortVideos")
project_description = _cfg.get(
    "project_description",
    "BuildshortVideos — AI-powered short video generator",
)
